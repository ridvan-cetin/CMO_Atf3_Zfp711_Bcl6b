...
# ==============================================================================
# r3dcol: A list containing multiple color palette options for different 
# visualization needs throughout the analysis

# Initialize empty list to store all color palettes
r3dcol <- list()

# cols_94: Large palette with 94 distinct colors for datasets with many categories
# Useful for visualizing high-resolution clustering
r3dcol$cols_94 <- c(
...
# 37 colors matching developmental biology conventions
...
# cols_4x3: 12 colors arranged as 4 color families x 3 shades each
# Useful for showing groups with subgroups
r3dcol$cols_4x3 <- c(
...
# cols_4x3b: Alternative 12-color palette with 4 families x 3 shades
r3dcol$cols_4x3b <- c(
  "#551153","#970098","#cc34cd","#c85b00","#f98517","#fdbf6f","#00603d","#008c5c","#33b983","#002f64",
  "#0050ae","#1077f3"
)

# cols_4x1: 4 colors for major group comparisons (e.g., 4 conditions)
r3dcol$cols_4x1 <- c(
...
# cols_3a: 3 colors for three-way comparisons
r3dcol$cols_3a <- c(
...
suppressMessages({
  ...
  # Seurat ecosystem packages
  library(Seurat)        # Core single-cell analysis package
  library(SeuratData)    # Access to example datasets
  library(SeuratDisk)    # Convert between Seurat and other formats (h5ad, loom)
  library(SeuratObject)  # S4 object classes for Seurat
  
  # Data cleaning packages
  library(SoupX)         # Ambient RNA removal from droplet-based scRNA-seq
  library(reticulate)    # Python interface for R (required by some tools)
  library(scDblFinder)   # Doublet detection using simulation-based approach
  
  # Visualization packages
  library(ggplot2)       # Grammar of graphics plotting
  library(cowplot)       # Combine multiple ggplots
  library(RColorBrewer)  # Color palettes for visualization
  library(gridExtra)     # Arrange multiple plots in grid
  library(ggExtra)       # Add marginal histograms to ggplots
  
  # Data handling packages
  library(hdf5r)         # Read/write HDF5 files
  library(Matrix)        # Sparse matrix operations
  library(dplyr)         # Data manipulation
  
  # Analysis packages
  library(gprofiler2)    # Gene ortholog conversion and enrichment
  library(clustree)      # Visualize clustering resolution relationships
})
...
# ==============================================================================
# SECTION 3: LOAD DATA FROM PREVIOUS STEP
# ==============================================================================

```{r, eval=FALSE}
# ==============================================================================
# Load Seurat Objects from Demultiplexing Step
# ==============================================================================
# These objects were created and processed in 1_demultiplexing.Rmd
# Load your saved Seurat objects here:
...
# ==============================================================================
# Ensure RNA Assay is Active (Not CMO)
# ==============================================================================
# After demultiplexing, the CMO assay might be set as default
# We need to switch to RNA for expression analysis
...
# ==============================================================================
# SECTION 4: ADD QUALITY CONTROL METRICS
# ==============================================================================
...
```{r, eval=FALSE}
# ==============================================================================
# Calculate and Add QC Metrics to Each Lane
# ==============================================================================

# --- LANE 1 ---
# log1p_nCount_RNA: Log-transformed total UMI counts (log1p = log(x+1))
# Adding 1 prevents log(0) and compresses the distribution for visualization
seurat_Lane1$log1p_nCount_RNA <- log1p(seurat_Lane1@meta.data$nCount_RNA)

# log1p_nFeature_RNA: Log-transformed number of detected genes
...
# High mitochondrial content indicates stressed/dying cells
# Pattern "^mt-" matches mouse mitochondrial genes (lowercase in mouse genome)
seurat_Lane1[["percent.mt"]] <- PercentageFeatureSet(seurat_Lane1, pattern = "^mt-")

# percent.ribo: Percentage of reads from ribosomal genes
# Pattern matches: Rpl* (large subunit), Rps* (small subunit), Mrp* (mitochondrial)
...
# ==============================================================================
# SECTION 5: SUBSET SINGLETS FROM DEMULTIPLEXING
# ==============================================================================
...
```{r,eval=FALSE}
# ==============================================================================
# Filter to Keep Only Singlet Cells
# ==============================================================================

# Set the active identity to the demultiplexing comparison column
# This column was created in the demultiplexing step
Idents(seurat_Lane1) <- "HTODemux_vs_CellRanger"

# subset(): Keep only cells classified as singlets by either or both methods
# "HTODemux_and_CellRanger": Both methods agree it's a singlet
# "unique_CellRanger": Only CellRanger calls it a singlet
# "unique_HTODemux": Only HTODemux calls it a singlet
# This excludes: Doublets and Negatives
L1 <- subset(seurat_Lane1, idents=c("HTODemux_and_CellRanger","unique_CellRanger","unique_HTODemux"))

# Repeat for other lanes:
# L2 <- subset(seurat_Lane2, idents=c("HTODemux_and_CellRanger","unique_CellRanger","unique_HTODemux"))
# L3 <- subset(seurat_Lane3, idents=c("HTODemux_and_CellRanger","unique_CellRanger","unique_HTODemux"))
# L4 <- subset(seurat_Lane4, idents=c("HTODemux_and_CellRanger","unique_CellRanger","unique_HTODemux"))
# L5 <- subset(seurat_Lane5, idents=c("HTODemux_and_CellRanger","unique_CellRanger","unique_HTODemux"))
```

# ==============================================================================
# SECTION 6: QC FILTERING WITH MAD (MEDIAN ABSOLUTE DEVIATION)
# ==============================================================================
...
```{r eval=FALSE}
# ==============================================================================
# Calculate MAD-Based Thresholds for Mitochondrial Content
# ==============================================================================
# MAD (Median Absolute Deviation) is a robust measure of variability
# Using MAD instead of SD makes thresholds resistant to outliers

# Extract metadata as a data frame for easier manipulation
Cell.QC.Stat <- L1@meta.data

# Calculate upper threshold for mitochondrial percentage
# Formula: median + 3 * MAD
# Cells above this have unusually high mitochondrial content (dying cells)
max.mito.thr <- median(Cell.QC.Stat$percent.mt) + 3*mad(Cell.QC.Stat$percent.mt)

# Calculate lower threshold for mitochondrial percentage
# Formula: median - 3 * MAD
# Usually not restrictive (often negative, so effectively no lower bound)
...
# ==============================================================================
# Visualize Mitochondrial Content Distribution (Before Filtering)
# ==============================================================================

# Create scatter plot: nFeature_RNA vs percent.mt
# Red dashed lines show the MAD thresholds
p1_mt <- ggplot(Cell.QC.Stat, aes(x=nFeature_RNA, y=percent.mt)) +
      geom_point() +
      # Add horizontal lines at the thresholds
      geom_hline(aes(yintercept = max.mito.thr), colour = "red", linetype = 2) +
      geom_hline(aes(yintercept = min.mito.thr), colour = "red", linetype = 2) +
      # Annotate with number of cells to be removed/retained
...
# ggMarginal: Add marginal histograms to show distributions
ggMarginal(p1_mt, type = "histogram", fill="lightgrey", bins=100)  

# ==============================================================================
# Filter Cells Based on Mitochondrial Thresholds
# ==============================================================================

# filter(): Keep only cells within the mitochondrial content thresholds
Cell.QC.Stat <- Cell.QC.Stat %>% 
  filter(percent.mt < max.mito.thr) %>% 
  filter(percent.mt > min.mito.thr)

# ==============================================================================
# Visualize After Mitochondrial Filtering
# ==============================================================================
...
# ==============================================================================
# Calculate MAD-5 Thresholds for Gene Count (nFeature_RNA)
# ==============================================================================

# Use log10 transformation for gene counts (better distribution)
# MAD-5: More lenient threshold, allows some low-quality cells for downstream filtering
...
# ==============================================================================
# Calculate MAD-3 Thresholds (Stricter, for Later Use)
# ==============================================================================

# MAD-3: Stricter threshold, will be used after doublet detection
...
# ==============================================================================
# Calculate MAD-5 Thresholds for UMI Count (nCount_RNA)
# ==============================================================================
...
# ==============================================================================
# Calculate MAD-3 Thresholds for UMI Count (Stricter)
# ==============================================================================
...
# ==============================================================================
# Visualize Gene vs UMI Count with MAD-5 Thresholds
# ==============================================================================

# Scatter plot of log10(nCount_RNA) vs log10(nFeature_RNA)
# Green lines: gene count thresholds
# Red lines: UMI count thresholds
...
# ==============================================================================
# Visualize with Both MAD-5 and MAD-3 Thresholds
# ==============================================================================

# Blue lines show the stricter MAD-3 thresholds for comparison
p1_nFnC3 <- ggplot(Cell.QC.Stat, aes(x=log10(nCount_RNA), y=log10(nFeature_RNA))) +
      geom_point() +
      geom_smooth(method="lm") +
      # MAD-5 thresholds (green/red)
      geom_hline(aes(yintercept = min.Genes.thr), colour = "green", linetype = 2) +
      geom_hline(aes(yintercept = max.Genes.thr), colour = "darkgreen", linetype = 2) +
      geom_vline(aes(xintercept = min.nUMI.thr), colour = "red", linetype = 2)+
      geom_vline(aes(xintercept = max.nUMI.thr), colour = "darkred", linetype = 2)+
      # MAD-3 thresholds (blue)
...
# ==============================================================================
# Apply MAD-5 Filtering
# ==============================================================================

# Keep cells that pass all four thresholds (MAD-5)
Cell.QC.Stat <- Cell.QC.Stat %>% 
  filter(log10(nFeature_RNA) > min.Genes.thr) %>%    # Above minimum gene threshold
  filter(log10(nCount_RNA) > min.nUMI.thr) %>%       # Above minimum UMI threshold
  filter(log10(nCount_RNA) < max.nUMI.thr)	%>%        # Below maximum UMI threshold
  filter(log10(nFeature_RNA) < max.Genes.thr)         # Below maximum gene threshold

# ==============================================================================
# Visualize After MAD-5 Filtering
# ==============================================================================
...
# ==============================================================================
# SECTION 7: GENE/UMI RATIO ANALYSIS (ANNOTATION ONLY)
# ==============================================================================
...
```{r eval=FALSE}
# ==============================================================================
# Fit Linear Model to Gene/UMI Relationship
# ==============================================================================

# lm(): Fit a linear regression model
# This models the expected relationship between UMI counts and gene counts
# Cells deviating significantly from this line may have quality issues
lm.model <- lm(data = Cell.QC.Stat, formula = log10(nFeature_RNA) ~ log10(nCount_RNA))

# ==============================================================================
# Visualize Gene/UMI Relationship with Regression Line
# ==============================================================================

# Orange line: Offset from regression line by -0.09 (identifies outliers)
p2_lnFlnC  <- ggplot(Cell.QC.Stat, aes(x=log10(nCount_RNA), y=log10(nFeature_RNA))) +
  geom_point() +
  geom_smooth(method="lm") +  # Blue regression line
  geom_hline(aes(yintercept = min.Genes.thr), colour = "green", linetype = 2) +
  geom_hline(aes(yintercept = max.Genes.thr), colour = "darkgreen", linetype = 2) +
  geom_vline(aes(xintercept = min.nUMI.thr), colour = "red", linetype = 2)+
  geom_vline(aes(xintercept = max.nUMI.thr), colour = "darkred", linetype = 2)+
  # Offset line to identify cells with abnormally low gene/UMI ratio
...
# ==============================================================================
# Annotate Cells Below the Gene/UMI Relationship Line
# ==============================================================================

# valideCells: TRUE if cell is above the offset line (normal gene/UMI ratio)
# FALSE if cell is below (potentially problematic, but NOT removed)
# Note: Erythroid cells naturally have low gene diversity, so we annotate only
...
# ==============================================================================
# Visualize Cells Colored by Validity
# ==============================================================================

p3_lnFlnC <- ggplot(Cell.QC.Stat, aes(x=log10(nCount_RNA), y=log10(nFeature_RNA))) +
  geom_point(aes(colour = valideCells)) +  # Color by validity status
  geom_smooth(method="lm") +
...
# IMPORTANT: We do NOT remove cells based on nUMI/nGene relationship
# Reason: Erythroid cells naturally fall below this line due to their expression profile
```

# ==============================================================================
# SECTION 8: ANNOTATE MAD-3 FOR LATER FILTERING
# ==============================================================================
...
```{r eval=FALSE}
# ==============================================================================
# Annotate Cells That Pass MAD-3 Filtering
# ==============================================================================

# Create a logical column indicating whether each cell passes MAD-3 thresholds
# This annotation will be used later for final filtering
Cell.QC.Stat$mad3_valide <- with(Cell.QC.Stat, {
  ...
  # Check each MAD-3 threshold condition
  condition1 <- log10_nFeature_RNA > min.Genes.thr3   # Above minimum genes
  condition2 <- log10_nCount_RNA > min.nUMI.thr3      # Above minimum UMI
  condition3 <- log10_nCount_RNA < max.nUMI.thr3      # Below maximum UMI
  condition4 <- log10_nFeature_RNA < max.Genes.thr3   # Below maximum genes

  # Cell passes if ALL conditions are TRUE
  condition1 & condition2 & condition3 & condition4
})
```

# ==============================================================================
# SECTION 9: SUBSET SEURAT OBJECT WITH QC-PASSED CELLS
# ==============================================================================
...
```{r eval=FALSE}
# ==============================================================================
# Apply QC Filter to Seurat Object
# ==============================================================================

# subset(): Keep only cells present in the filtered Cell.QC.Stat data frame
# cells parameter: Vector of cell barcodes to retain
# Cell.QC.Stat$sample_id contains the barcodes of cells that passed MAD-5 filtering
...
```{r eval=FALSE}
# ==============================================================================
# Add QC Annotations to Seurat Metadata
# ==============================================================================

# Extract the validity annotations from filtered data
...
# AddMetaData(): Adds new columns to Seurat object metadata
...
# ==============================================================================
# SECTION 10: AMBIENT RNA REMOVAL WITH SOUPX
# ==============================================================================
...
```{r eval=FALSE}
# ==============================================================================
# Load Raw Data for SoupX
# ==============================================================================

# SoupX requires the raw (unfiltered) count matrix to estimate the ambient RNA profile
# This should be saved from the data loading step
...
```{r eval=FALSE}
# ==============================================================================
# Prepare Data for SoupX: Quick Clustering
# ==============================================================================
# SoupX performs better when provided with cluster information
# This allows it to estimate contamination more accurately

# Repeat this process for all lanes (L1 through L5)

# Backup original counts before SoupX modification
# CreateAssayObject(): Stores the original counts for reference
L1[["original.counts"]] <- CreateAssayObject(counts = L1@assays$RNA$counts)

# Quick clustering pipeline for SoupX (not the final clustering)
L1 <- NormalizeData(L1, verbose = FALSE)
# Find 4000 variable features using variance stabilizing transformation
L1 <- FindVariableFeatures(object = L1, nfeatures = 4000, verbose = FALSE, selection.method = 'vst')
L1 <- ScaleData(L1, verbose = FALSE)
# Use 80 PCs for more complete representation
L1 <- RunPCA(L1, npcs = 80, verbose = FALSE)
L1 <- FindNeighbors(L1, dims = 1:80, verbose = FALSE)
# High resolution (1.5) for more clusters - helps SoupX
L1 <- FindClusters(L1, resolution = 1.5, verbose = FALSE)
L1 <- RunUMAP(L1, dims = 1:80, verbose = FALSE)
# Store cluster assignments for SoupX
...
# SoupChannel(): Create SoupX channel object
# First argument: Raw (unfiltered) counts - contains all droplets including empty ones
# Second argument: Filtered counts - contains only cell-containing droplets
# calcSoupProfile = FALSE: We'll estimate the soup profile in the next step
sc = SoupChannel(data_l1$`Gene Expression`, L1@assays$RNA$counts, calcSoupProfile = FALSE)

# estimateSoup(): Estimate the ambient RNA profile from empty droplets
sc = estimateSoup(sc)

# setClusters(): Provide cluster information to improve contamination estimation
sc <- setClusters(sc, L1$soup_group)

# autoEstCont(): Automatically estimate the contamination fraction
# doPlot=TRUE: Generate diagnostic plots
sc <- autoEstCont(sc, doPlot=TRUE)

# View the top genes in the estimated soup profile
# These are the genes most contaminating the data
head(sc$soupProfile[order(sc$soupProfile$est, decreasing = TRUE), ], n = 100)

# plotMarkerDistribution(): Visualize marker gene distributions
...
# adjustCounts(): Remove estimated ambient RNA contamination
# roundToInt = TRUE: Round corrected counts to integers (required for Seurat)
out = adjustCounts(sc, roundToInt = T)

# Store corrected counts in the Seurat object
L1@assays$RNA$soupXauto <- out

# Calculate and display the percentage of counts retained after SoupX
# This shows how much contamination was removed
sum(L1@assays$RNA$soupXauto)/sum(L1@assays$original.counts@counts)*100 

# ==============================================================================
# Replace Original Counts with SoupX-Corrected Counts
# ==============================================================================

# Update the RNA counts layer with corrected counts
L1@assays$RNA@layers$counts <- L1@assays$RNA@layers$soupXauto
# Remove the temporary soupXauto layer
...
# ==============================================================================
# SECTION 11: DOUBLET DETECTION WITH SCDBL FINDER
# ==============================================================================
...
# scDblFinder(): Identifies doublets using a simulation-based approach
# GetAssayData(): Extract the count matrix from the Seurat object
# slot="counts": Use raw counts (required by scDblFinder)
# dbr=0.2: Expected doublet rate of 20% (conservative estimate)
# clusters: Provide cluster information for more accurate detection
sce_L1 <- scDblFinder(GetAssayData(L1, slot="counts", dbr=0.2), clusters=Idents(L1))

# View the number of singlets and doublets detected
table(sce_L1$scDblFinder.class)

# Add scDblFinder results to Seurat metadata
# scDblFinder_score: Continuous score (0-1) indicating doublet probability
L1$scDblFinder_score <- sce_L1$scDblFinder.score
# scDblFinder_class: Binary classification (singlet/doublet)
...
```{r eval=FALSE}
# ==============================================================================
# Apply Manual Stricter Threshold for Doublet Detection
# ==============================================================================

# scDblFinder calculates an automatic threshold, but we use a stricter 0.25
# ifelse(): Create binary classification based on score threshold
# Cells with score > 0.25 are classified as doublets
Lane0$scDblFinder_manual025 <- ifelse(Lane0@meta.data$scDblFinder_score > 0.25, "doublet", "singlet")
```

```{r eval=FALSE}
# ==============================================================================
# Visualize scDblFinder Scores by Cluster
# ==============================================================================

# VlnPlot(): Create violin plot of doublet scores across clusters
# Red line: Average threshold calculated by software (~0.37)
# Blue line: Alternative threshold at 0.30
# Purple line: Our stricter threshold at 0.25
VlnPlot(Lane0, features="scDblFinder_score", group.by="cluster_4") + 
  geom_hline(yintercept=0.37, linetype="dashed", color = "red", size=1) + 
  geom_hline(yintercept=0.30, linetype="dashed", color = "blue", size=1) + 
  geom_hline(yintercept=0.25, linetype="dashed", color = "purple", size=1) &
  NoLegend()
```
...
```{r eval=FALSE}
# ==============================================================================
# Merge All Lanes into Single Seurat Object
# ==============================================================================

# merge(): Combine multiple Seurat objects
# Cell barcodes are made unique by appending lane identifiers
Lane0 <- merge(L1, y=c(L2,L3,L4,L5))

# DietSeurat(): Remove unnecessary data to reduce object size
# Keeps only essential slots (counts, metadata, reductions)
Lane0 <- DietSeurat(Lane0)

# JoinLayers(): Merge fragmented layers into single layers
# Required after merging in Seurat v5
Lane0 <- JoinLayers(Lane0)
```

# ==============================================================================
# SECTION 13: RE-PROCESS MERGED DATA
# ==============================================================================
...
```{r eval=FALSE}
# ==============================================================================
# Standard Seurat Processing Pipeline on Merged Data
# ==============================================================================

# NormalizeData(): Log-normalize the merged data
# normalization.method = "LogNormalize": log1p(count/total * scale.factor)
# scale.factor = 10000: Standard scaling factor
Lane0 <- NormalizeData(Lane0, normalization.method = "LogNormalize", scale.factor = 10000, verbose = F)

# Get all gene names for scaling
all.genes <- rownames(Lane0)

# ScaleData(): Scale all genes (not just variable features)
Lane0 <- ScaleData(Lane0, features = all.genes, verbose = F)

# FindVariableFeatures(): Identify 4000 most variable genes
Lane0 <- FindVariableFeatures(Lane0, selection.method = 'vst', nfeatures = 4000, verbose = F)

# RunPCA(): Perform PCA using 75 principal components
# reduction.name = "lpca": Name this reduction for reference
Lane0 <- RunPCA(Lane0, features = VariableFeatures(Lane0), npcs = 75, verbose = F, 
                reduction.name = "lpca")

# FindNeighbors(): Build k-nearest neighbor graph using 75 PCs
Lane0 <- FindNeighbors(Lane0, reduction = "lpca", dims = 1:75)

# RunUMAP(): Generate UMAP embedding
# spread = 2: Controls how spread out the clusters are
Lane0 <- RunUMAP(Lane0, reduction = "lpca", dims = 1:75, spread = 2, reduction.name = "umap_1")

# FindClusters(): Cluster cells at high resolution
# resolution = 4: High resolution for fine-grained clusters
# algorithm = 4: Leiden algorithm (more accurate than Louvain)
# method = 'igraph': Use igraph implementation
# n.start = 100, n.iter = 100: Multiple random starts for stability
Lane0 <- FindClusters(Lane0, resolution = 4, algorithm = 4, method = 'igraph', 
                      n.start = 100, n.iter = 100, cluster.name = "cluster_4", verbose = F)

# ==============================================================================
# Re-calculate Cell Cycle Scores on Merged Data
# ==============================================================================

# Get updated cell cycle genes from Seurat
s.genes.updated.2019 <- cc.genes.updated.2019$s.genes
g2m.genes.updated.2019 <- cc.genes.updated.2019$g2m.genes

# Convert human cell cycle genes to mouse orthologs
# gorth(): Gene ortholog conversion from gprofiler2
...
# Add additional S-phase genes not in the standard list
mmus_s_2 <- c(mmus_s, "Pold3", "Atad2")

# Display gene counts for reference
...
# CellCycleScoring(): Assign cell cycle phase to each cell
# Adds: S.Score, G2M.Score, Phase metadata columns
Lane0 <- CellCycleScoring(Lane0, s.features = s.genes, g2m.features = g2m.genes, set.ident = T)
```

# ==============================================================================
# SECTION 14: FINAL CELL FILTERING
# ==============================================================================
...
# --- Filter 1: Remove HTODemux Doublets and Negatives ---
# Keep only cells classified as Singlets by HTODemux
Idents(Lane0) <- "CMO_classification.global"
# invert=T: Remove the specified identities instead of keeping them
Lane0 <- subset(Lane0, idents = c("Doublet","Negative"), invert = T)

# --- Filter 2: Keep Only CellRanger Singlets ---
# Subset for cells that CellRanger also classified as singlets
Idents(Lane0) <- "CellRanger_Assignment_global"
Lane0 <- subset(Lane0, ident = "singlet")

# --- Filter 3: Apply MAD-3 Filter ---
# Remove cells that don't pass the stricter MAD-3 quality thresholds
Idents(Lane0) <- "MAD3_filter"
table(Idents(Lane0))  # Show counts before filtering
Lane0 <- subset(Lane0, idents = "TRUE")

# --- Filter 4: Remove scDblFinder Doublets (Manual Threshold) ---
# Create new classification with stricter 0.20 threshold
...
# ==============================================================================
# SECTION 15: CELL CYCLE REGRESSION AND DIMENSIONALITY REDUCTION
# ==============================================================================
...
# Create working copy of the Seurat object
cmo <- Lane0

# ScaleData with regression: Remove cell cycle variation from expression data
# vars.to.regress: Variables to regress out
# S.Score and G2M.Score: Cell cycle scores calculated earlier
# features = rownames(cmo): Scale ALL genes (required for proper regression)
# Note: This takes significant computation time
cmo <- ScaleData(cmo, vars.to.regress = c("S.Score", "G2M.Score"), features = rownames(cmo))

# ==============================================================================
# Re-run Dimensionality Reduction After Cell Cycle Regression
# ==============================================================================

# Find variable features again (on regressed data)
cmox <- FindVariableFeatures(cmox, selection.method = "vst", nfeatures = 4000)

# Run PCA on cell-cycle-regressed data
# reduction.name = "cc_pca": Name indicates cell-cycle-corrected PCA
cmox <- RunPCA(cmox, features = VariableFeatures(cmox), reduction.name = "cc_pca", npcs = 75)

# ==============================================================================
# Generate UMAP Embeddings (2D and 3D)
# ==============================================================================

# 3D UMAP for visualization tools
# n.neighbors = 50: Number of neighbors for graph construction
# n.epochs = 1000: More epochs for stable embedding
# n.components = 3: Three-dimensional embedding
cmox <- RunUMAP(cmox, reduction = "cc_pca", dims = 1:75, spread = 1, n.neighbors = 50,
                n.epochs = 1000, n.components = 3, reduction.name = "cc_umap_3xx_3d")

# 2D UMAP for standard visualization
cmox <- RunUMAP(cmox, reduction = "cc_pca", dims = 1:75, spread = 1, n.neighbors = 50,
                n.epochs = 1000, n.components = 2, reduction.name = "cc_umap_3xx")

# ==============================================================================
# Clustering on Cell-Cycle-Regressed Data
# ==============================================================================

# Build neighbor graph
# k.param = 50: Number of neighbors
cmox <- FindNeighbors(cmox, reduction = "cc_pca", dims = 1:75, k.param = 50)

# Find clusters using Leiden algorithm
# resolution = 1.25: Moderate resolution for initial clustering
...
# ==============================================================================
# Run t-SNE for Alternative Visualization
# ==============================================================================

# RunTSNE(): Alternative dimensionality reduction method
cmox <- RunTSNE(cmox, reduction = "cc_pca", dims = 1:75, reduction.name = "cc_tsne")

```

# ==============================================================================
# SECTION 16: SUBCLUSTERING SPECIFIC CLUSTERS
# ==============================================================================
...
```{r eval=FALSE}
# ==============================================================================
# Subcluster Specific Clusters for Higher Resolution
# ==============================================================================
# FindSubCluster(): Performs sub-clustering within a specified cluster
# Useful for identifying subpopulations within heterogeneous clusters

# Set the active identity to the main clustering
Idents(cmox) <- "cc_Leiden_1.25_x100"

# --- Subcluster Cluster 16 ---
# graph.name = "RNA_snn": Use the shared nearest neighbor graph
# resolution = 0.25: Low resolution for few subclusters
...
cmox <- FindSubCluster(
  ...
  resolution = 0.2,  # Even lower resolution
  algorithm = 4
)
...
cmox <- FindSubCluster(cmox, cluster = "18", graph.name = "RNA_snn", 
                       subcluster.name = "sub.cluster_18", resolution = 0.25, algorithm = 4)

# --- Subcluster Cluster 21 ---
...
# ==============================================================================
# SECTION 17: CLUSTER RENAMING AND ANNOTATION
# ==============================================================================
...
```{r eval=FALSE}
# ==============================================================================
# Rename Clusters with Descriptive Names (High Resolution)
# ==============================================================================

# Set active identity to the final subclustering result
Idents(cmox) <- "sub.cluster_21"

# Define new descriptive cluster names
# Names based on marker gene expression and cell type identification
...
# Apply new names to cluster identities
names(new.cluster.ids) <- levels(cmox)
cmox <- RenameIdents(cmox, new.cluster.ids)
# Store high-resolution names in metadata
cmox$cc_clusters <- Idents(cmox)

# Apply renaming again and store
names(new.cluster.ids) <- levels(cmox)
cmox <- RenameIdents(cmox, new.cluster.ids)
cmox$cc_cluster_low <- Idents(cmox)

# ==============================================================================
# Create Mid-Resolution Cluster Names
# ==============================================================================

# Simplified names grouping related subclusters
new.cluster.ids <- 
...
names(new.cluster.ids) <- levels(cmox)
cmox <- RenameIdents(cmox, new.cluster.ids)
# Store mid-resolution names
cmox$cc_cluster_lower <- Idents(cmox)

# ==============================================================================
# Clean Up Reduction Names
# ==============================================================================

# Rename reductions for cleaner access
...
# ==============================================================================
# SECTION 18: CREATE SUBSET OBJECTS BY MAJOR GROUPS
# ==============================================================================
...
```{r eval=FALSE}
# ==============================================================================
# Create Separate Seurat Objects for Major Cell Groups
# ==============================================================================

# Early Differentiation Group: Pluripotent and early lineage cells
Early <- subset(cmox, idents = c("Naive_ES","PS","PGC","Epiblast","Ectoderm","Endoderm"))

# Late Mesoderm Group: Mesoderm and Hox-patterned cells
Mesoderm <- subset(cmox, idents = c("Mesoderm","Hoxa","Hoxb","Hoxab"))

# Hematopoietic Group: Blood and endothelial lineages
...
# --- Process Early Differentiation Subset ---
...
# ==============================================================================
# SECTION 19: CLUSTERING RESOLUTION ANALYSIS
# ==============================================================================
...
```{r eval=FALSE}
# ==============================================================================
# Visualize Clustering Resolution Hierarchy
# ==============================================================================

# clustree(): Visualize how clusters change across resolutions
# prefix = "cl_": Column prefix for cluster assignments at different resolutions
# layout = "sugiyama": Hierarchical layout
# This helps identify stable vs. unstable clusters across resolutions
clustree(cmo, prefix = "cl_", node_text_angle = 40, layout = "sugiyama", 
         node_text_size = 4.5, edge_width = 0.5, node_colour = "lightblue") +
  ylim(0.9, 3.1) & 
  NoLegend()
```

# ==============================================================================
# SECTION 20: HOX GENE MODULE SCORING
# ==============================================================================

```{r eval=FALSE}
# ==============================================================================
# Create Hox Gene Lists and Calculate Module Scores
# ==============================================================================

# Initialize list to store Hox gene families
hox_genes <- list()

# grep(): Find genes matching Hox gene patterns
# value = TRUE: Return gene names, not indices
hox_genes$hoxa_genes <- grep("^Hoxa", rownames(cmo), value = TRUE)  # Hoxa cluster genes
hox_genes$hoxb_genes <- grep("^Hoxb", rownames(cmo), value = TRUE)  # Hoxb cluster genes
hox_genes$hoxc_genes <- grep("^Hoxc", rownames(cmo), value = TRUE)  # Hoxc cluster genes
hox_genes$hoxd_genes <- grep("^Hoxd", rownames(cmo), value = TRUE)  # Hoxd cluster genes

# AddModuleScore(): Calculate average expression of gene sets per cell
# Returns a score for each gene list in the 'features' parameter
cmo <- AddModuleScore(cmo, features = hox_genes, name = "Score")

# Rename score columns to more descriptive names
cmo$score_hoxa <- cmo$Score1
cmo$score_hoxb <- cmo$Score2
cmo$score_hoxc <- cmo$Score3
cmo$score_hoxd <- cmo$Score4

# Remove original Score columns
cmo$Score1 <- NULL
cmo$Score2 <- NULL
cmo$Score3 <- NULL
cmo$Score4 <- NULL
...
# DotPlot for individual Hox genes
DotPlot(cmo, features = hox_genes, scale.by = "size") + 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size = 15)) +
  geom_hline(yintercept = c(7.5, 9.5), color = "red") +
  xlab("") + ylab("")

# DotPlot for Hox module scores
...
# ==============================================================================
# SECTION 21: MARKER GENE LISTS FOR CELL TYPE ANNOTATION
# ==============================================================================
...
```{r eval=FALSE}
# ==============================================================================
# Define Marker Gene Lists for Each Cell Type
# ==============================================================================
# These markers are used for cell type identification and annotation

markers <- list()

# --- Early Differentiation Markers ---
markers$Naive <- c("Morc1","Ooep","Sox2","Esrrb","Klf2","Zfp42")  # Naive pluripotent ES cells
markers$Epib <- c("Pim2","Tcfl5","Kcnj3","Pou3f1","Nanog","Pou5f1")  # Epiblast
markers$PST <- c("T","Igfbp3","Ppp4r4","Appl1","Rab18","Fgf8")  # Primitive Streak (T/Brachyury+)
...
# --- PGC and Mesoderm Markers ---
markers$PGC <- c("Ttc28","Dnd1","Hmcn1","Prdm1")  # Primordial Germ Cells
markers$PSMesp1 <- c("Mesp1","Limch1","Wnt5a")  # Primitive Streak/Mesp1+

# --- Endoderm Markers ---
markers$EnXFgf5 <- c("Fgf5","Fgf10","Lhx1","Gsc","Sox17","Dgkk","Otx2","Cfc1")
markers$EnXTtr <- c("Ttr","Spink1","Ctsh","Clic6","Dpp4","S100g","Car4")  # Visceral Endoderm

# --- Mesoderm Markers ---
markers$MesoMixl1 <- c("Mixl1","Eomes","Wnt3","Tbx3","Rftn1","Grrp1","Car14")

# --- Hoxb+ Mesoderm Markers ---
...
# --- Interferon-Responsive Markers ---
markers$baIfit1 <- c("Ifit1","Isg15","Gbp7","Ifit3","Usp18","Rsad2","Oasl2")

# --- Blood Progenitor Markers ---
markers$BPr <- c("Tal1","Lmo2","Runx1","Slc39a8","Fgf3","Myb","Slc32a1","Rpp25")

# --- Erythroid Markers (Maturation Stages) ---
markers$Ery1a <- c("Glrx5","Tspan33","Nt5dc2","Mthfd2","Asns")  # Early erythroid
markers$Ery1b <- c("Tpx2","Cpox","Cited4","Ssx2ip","Cenpk")  # Early erythroid (cycling)
markers$Ery2 <- c("Slc4a1","Ermap","Hemgn","Rhag","Acp5","Kel")  # Intermediate erythroid
markers$Ery3 <- c("Hbb-y","Hba-a2","Alas2","Slc25a21","Gypa","March3","Mllt3")  # Mature erythroid

# --- Megakaryocyte Markers ---
markers$MegP1 <- c("Rab27b","Gp1bb","Gp9","Treml1","Sla2","Gnaz","Mpl","Mrvi1")  # Early MkP
markers$MegP2 <- c("Hsd3b1","Thbs1","Gp1bb","Gp5","Tubb1","Rasl10a","Cd226")  # Intermediate MkP
markers$MegP3 <- c("Thbs1","Gp1bb","Gp9","Treml1","Alox12","Vwf")  # Mature MkP
...
# --- EMP (Erythro-Myeloid Progenitor) Markers ---
...
# ==============================================================================
# Create Summary DataFrame of Markers
# ==============================================================================

# Get vector names (cell types)
vector_names <- names(markers)

# Merge genes in each list into comma-separated strings
merged_genes <- sapply(markers, function(x) paste(x, collapse = ", "))

# Create data frame for reference/export
...
# ==============================================================================
# Create DotPlots of Marker Genes
# ==============================================================================

# First batch of markers (genes 1-123)
fig_dot_1 <-
  DotPlot(cmo, features = vec_uniq[1:123]) + 
  theme(axis.text.x = element_text(angle = 60, vjust = 1, hjust = 1)) +
  coord_flip() + xlab("") + ylab("") + coord_flip() + NoLegend() +
  # Add vertical lines to separate cell type groups
  geom_vline(xintercept = c(18.5, 41.5, 63.5, 83.5, 106.5), color = "grey") +
  xlab("") + ylab("")

# Second batch of markers (genes 124-238)
fig_dot_2 <- DotPlot(cmo, features = vec_uniq[124:238]) + 
  theme(axis.text.x = element_text(angle = 60, vjust = 1, hjust = 1)) +
  coord_flip() + xlab("") + ylab("") + 
  geom_vline(xintercept = c(20.5, 38.5, 61.5, 78.5, 98.5), color = "grey") +
  NoLegend() + coord_flip()
...
# ==============================================================================
# SECTION 22: SUMMARY DOTPLOT WITH CELL TYPE BOXES
# ==============================================================================

```{r eval=FALSE}
# ==============================================================================
# Create Summary DotPlot with Cell Type Region Highlights
# ==============================================================================

# Selected key markers for overview visualization
# geom_rect(): Add colored rectangles to highlight cell type regions
DotPlot(cmo, features = c("Zfp42","Pim2","T","Tacstd2","Sfn","Cldn3","Anxa1","Dnd1","Mesp1","Sox17",
                          "Fgf5","Ttr","Mixl1","Hoxb2","Fgf15","Cacna1d","Nrxn3","Stard8","Upk3b",
                          "Plxna4","Hoxa9","Greb1","Dach1","Actc1","Igfbp7","Wnt2","Fgf12","Ifit1",
                          "Tal1","Gypa","Hba-a1","Hba-a2","Alas2","Plek","Fermt3","Vwf","Icam2",
                          "Gadd45g","Cdh5","Cyp26b1","Rsad2","Ptprc","Nrg1","Il17ra","Ccl4","Sepp1"), 
        group.by = "cl_3") + 
  theme(axis.text.x = element_text(angle = 60, vjust = 1, hjust = 1, size = 15)) +
  # Add colored boxes around cell type regions
...
# ==============================================================================
# SECTION 23: CELL CYCLE PROPORTION ANALYSIS
# ==============================================================================

```{r eval=FALSE}
# ==============================================================================
# Analyze Cell Cycle Phase Distribution Across Clusters
# ==============================================================================

# Get cluster level factors for consistent ordering
cl4l <- levels(cmo$cl_3)     # Subcluster level
cl4l2 <- levels(cmo$cl_2)    # Cluster level
cl4l1 <- levels(cmo$cl_1)    # Group level
ccpl <- levels(cmo$Phasel)   # Cell cycle phases

# Display level orders
cl4l
ccpl
cl4l2
cl4l1
...
# ==============================================================================
# Create Cell Cycle Proportion Plots at Different Resolutions
# ==============================================================================

# plotCellTypeProps(): From speckle package, shows proportions of one variable across another

# Subcluster level (cl_3) - Highest resolution
cc_speckle_cl3 <- plotCellTypeProps(sample = cmo$cl_3, clusters = cmo$Phasel) + 
  theme(axis.text.x = element_text(angle = 60, vjust = 1, hjust = 1)) + 
  scale_x_discrete(limits = cl4l) + xlab("") + 
  geom_hline(yintercept = c(), color = "grey") +
  scale_fill_manual("Cell Cycle", values = r3dcol$c3, limits = ccpl) + ylab("")

# Cluster level (cl_2) - Medium resolution
cc_speckle_cl2 <- plotCellTypeProps(sample = cmo$cl_2, clusters = cmo$Phasel) + 
  theme(axis.text.x = element_text(angle = 35, vjust = 1, hjust = 1)) + 
  scale_x_discrete(limits = cl4l2) + xlab("") + 
  geom_hline(yintercept = c(), color = "grey") +
  scale_fill_manual("Cell Cycle", values = r3dcol$c3, limits = ccpl) + ylab("") + NoLegend()

# Group level (cl_1) - Lowest resolution
cc_speckle_cl1 <- plotCellTypeProps(sample = cmo$cl_1, clusters = cmo$Phasel) + 
  theme(axis.text.x = element_text(angle = 35, vjust = 1, hjust = 1)) + 
  scale_x_discrete(limits = cl4l1) + xlab("") + 
  geom_hline(yintercept = c(), color = "grey") + 
  scale_fill_manual("Cell Cycle", values = r3dcol$c3, limits = ccpl) + ylab("") + NoLegend()
...
# ==============================================================================
# SECTION 24: SESSION INFORMATION
# ==============================================================================
...
# ==============================================================================
# sessionInfo(): Displays R version, platform, and loaded package versions
sessionInfo()
```
